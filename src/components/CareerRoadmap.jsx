    ...
    <div className="bg-white p-4 rounded-lg shadow mb-4">
        ...
        <div className="flex flex-wrap">
            {milestone.resources.map((resource, index) => (
                <span key={index} className="text-xs mr-2 mb-2 px-2 py-1 bg-blue-100 text-blue-800 rounded-full">{resource
                }</span>))}
        </div>
...
const CareerRoadmap = ({ careerPathSuggestion }) => {
    return (
        <div className="bg-white p-6 rounded-lg shadow mt-6">
            ...
            <p className="mb-6">{careerPathSuggestion.description}</p>
